...
const useSQLiteDB = () => {
  ...
  useEffect(() => {
    const initializeDB = async () => {
      ...
      const isConn = (await sqlite.current.isConnection("system_db", false))
        .result;

      if (ret.result && isConn) {
        db.current = await sqlite.current.retrieveConnection("system_db", false);
      } else {
  ...
  const initializeTables = async () => {
    ...
    `;
      console.log("Query: ", queryCreateTable);
